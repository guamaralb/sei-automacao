...
def clicar_iniciar_processo(driver: webdriver.Chrome):
    for index in range(3):
        try:
            span_iniciar_processo = WebDriverWait(driver, 20).until(
...
def selecionar_tipo_processo(driver: webdriver.Chrome, tipo_processo: str):
    a_exibir_todos_tipos_processo = WebDriverWait(driver, 20).until(
    ...
    for _ in range(3):  # tenta até 3 vezes
        try:
            a_tipo_processo = WebDriverWait(driver, 20).until(
...
def preencher_especificacao_processo(driver: webdriver.Chrome, especificacao: str):

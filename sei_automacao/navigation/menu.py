...
def clicar_iniciar_processo(driver: webdriver.Remote) -> None:
    for index in range(3):
        try:
            span_iniciar_processo: WebElement = WebDriverWait(
                driver, 10
            ).until(
                EC.element_to_be_clickable((
                    By.XPATH,
                    "//span[contains(text(), 'Iniciar Processo')]",
                ))
            )
            ...
            WebDriverWait(driver, 10).until(
                EC.presence_of_element_located((
                    By.XPATH,
                    "//label[contains(text(), 'Escolha o Tipo do Processo:')]",
                ))
            )
            return
            ...
            print(
                f'Falha na tentativa {index + 1}/3 de iniciar processo: '
                f'{type(e).__name__} -> {e}'
            )
            time.sleep(2)

    raise Exception(
        "Falha ao clicar em 'Iniciar Processo' após várias tentativas"
    )


def selecionar_tipo_processo(
    driver: webdriver.Remote, tipo_processo: str
) -> None:
    a_exibir_todos_tipos_processo: WebElement = WebDriverWait(
        driver, 10
    ).until(EC.element_to_be_clickable((By.ID, 'ancExibirTiposProcedimento')))
    ...
    for _ in range(3):
        try:
            a_tipo_processo: WebElement = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable((
                    By.XPATH,
                    f"//a[normalize-space(text())='{tipo_processo}']",
                ))
            )
...
def preencher_especificacao_processo(
    driver: webdriver.Remote, especificacao: str
) -> None:
